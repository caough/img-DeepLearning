...
class BasicBlock(layers.Layer):
    ...
    def __init__(self, out_chancel, strides=1, downsample=None, **kwargs):
        super(BasicBlock, self).__init__(*kwargs)
        self.conv1 = layers.Conv2D(out_chancel, kernel_size=3, strides=strides, padding="SAME", use_bias=False)
        self.bn1 = layers.BatchNormalization(momentum=0.9, epsilon=1e-5)

        self.conv2 = layers.Conv2D(out_chancel, kernel_size=3, strides=1, padding="SAME", use_bias=False)
...
class Bottleneck(layers.Layer):
    ...
    def __init__(self, out_channel, strides=1, downsample=None, **kwargs):
        ...
        self.conv2 = layers.Conv2D(out_channel, kernel_size=3, padding="SAME", strides=strides, use_bias=False, name="conv2")
...
def _resnet(block, blocks_num, im_width=224, im_height=224, num_classes=1000, include_top=True):
    ...
    x = _make_layer(block, x.shape[-1], 128, blocks_num[1], strides=2, name="block1")(x)
    x = _make_layer(block, x.shape[-1], 256, blocks_num[2], strides=2, name="block1")(x)
    x = _make_layer(block, x.shape[-1], 512, blocks_num[3], strides=2, name="block1")(x)
